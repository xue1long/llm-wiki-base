...
- **Server runtime coverage.** `tests/test_server/test_app_lifespan.py` exercises `/health`, real `.llm-wiki/project.json` discovery, provider/embed stubs, zero-network startup, shutdown close, and embedding-singleton restoration in isolated temporary config/CWD state. Import-time or process-start failures still require the smoke test: after refactors touching `src/server/`, `src/cli.py`, or top-level imports of `src/wiki/`, run `python -m src.cli serve --port <free>` and curl `/health`.
