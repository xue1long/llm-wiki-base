...
| 按钮 | 功能 | 后端 API | 说明 |
|------|------|----------|------|
| **质量徽标 / 质** | 点击 RAW 文件行质量等级徽标打开详细质检报告 | `GET /api/v1/projects/{id}/quality?source_path=...` | 报告布局对应 `web/prototypes/raw-quality-report.html` 原型 |
| **× 关闭** | 关闭质检弹窗 | — | 点击遮罩层也可关闭 |
| **关闭**（报告底部） | 关闭质检弹窗 | — | |
| **重新摄取**（报告底部） | 关闭报告并对该文件重新执行摄取流水线 | `POST /api/v1/projects/{id}/reingest` | 报告底部按钮，行为同文件行”重新摄取” |

质检报告模态框分区（对应设计文档 `docs/superpowers/specs/2026-08-13-raw-quality-report-design.md`）：

- 头部：整体判定 Tag（通过 / 存在改进项 / 未通过）+ 来源文件 + 最近摄取时间
- 汇总卡片：整体结果、通过、需改进、待审核、已隔离（A/B/C 页面分布）
- 来源上下文：RAW 文件、摄取任务 ID、生成时间、生成页面数
- 生成的 Wiki 页面：页面级 A/B/C 质量分级列表
- 页面质量检查：占位页检测 / 证据覆盖 / 来源追溯 / 关系引用 等分项
- 内容指标：质量等级、源文件大小、核心结论、证据引用、分块数、页面数
- Pipeline 时间线：Collector → Analyzer → Reviewer → Promoter → Generator → Writer
- 审核项：待审核 ReviewItem + 隔离页面（quarantine）
- 无报告 / 接口异常时降级显示文件基础信息
...
`GET /api/v1/projects/{id}/book-wiki` 获取目录和章节信息，右侧面板展示 release、范围、覆盖率、source appendix、来源、关系和质量摘要。
...
| 按钮 | 位置行号 | 功能 | 后端 API | 说明 |
|------|----------|------|----------|------|
| **项目实例下拉框** | 状态页工具栏 | 选择要查看的项目实例并刷新项目生命周期状态 | `GET /api/v1/projects` + `POST /api/v1/projects/{id}/select` + `GET /api/v1/projects/{id}/status-summary` | 复用全局项目列表和切换逻辑 |
...
| 面板 | 后端 API | 说明 |
|------|----------|------|
| **RAW / KC / Wiki / Book 状态卡片** | `GET /api/v1/projects/{id}/status-summary` | 展示当前项目各阶段状态数量 |
| **来源明细** | `GET /api/v1/projects/{id}/status-summary` | 展示每个来源的 RAW、KC、Wiki、Book 和总状态 |

状态汇总 API 只读项目 `.index/lineage/state.db`，不会修改摄取、KC、Wiki 或 Book 数据。队列面板仍读取全局 `GET /api/v1/queue/status`，不属于项目生命周期汇总。
