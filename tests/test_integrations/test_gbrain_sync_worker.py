...
def test_import_command_is_fixed_to_project_wiki_and_source(tmp_path):
    ...
    assert calls[2][0][4] != str(tmp_path / "wiki")
    assert calls[2][0][5:-1] == command[5:]
    assert calls[2][0][-1] == "--allow-noncanonical-root"
...
def test_run_incremental_sync_updates_remote_and_keeps_ready_state(tmp_path, monkeypatch):
    _project(tmp_path)
    page_dir = tmp_path / "wiki" / "sources"
    page_dir.mkdir(parents=True)
    page = page_dir / "new.md"
    ...
    calls = []
    monkeypatch.setattr(
        "src.searcher.gbrain_mcp.run_mcp_mutation",
        lambda _runtime, source_id, operation, slug, content: calls.append(
            (operation, source_id, slug, content)
        ),
    )

    result = run_incremental_sync(tmp_path, tmp_path / "runtime")
...
def test_reconcile_restores_tombstone_before_upsert(tmp_path):
    ...
    assert result.success is True
    assert calls == [
        ("restore", config.source_id, "wiki/sources/old", None),
        ("upsert", config.source_id, "wiki/sources/old", "restored"),
    ]
