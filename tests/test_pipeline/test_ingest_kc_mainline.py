...
async def test_run_ingest_enters_kc_before_formal_commit(
    ...
    async def fake_compile_source(source: str, *, content: bytes, candidate_json: str) -> dict:
        called.append({"source": source, "content": content, "candidate_json": candidate_json})
        return {
    ...
    assert called[0]["content"] == raw.read_bytes()
