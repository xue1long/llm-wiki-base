...
def test_resolve_cli_project_preserves_return_shapes_and_mode(monkeypatch):
    ...
    monkeypatch.setattr(lib_project, "resolve_project", resolve_with_paths)
    monkeypatch.setattr(lib_project, "resolve_ctx_only", resolve_context)

    assert lib_project.resolve_cli_project("path", by_id_only=False) == (ctx, paths)
    assert lib_project.resolve_cli_project("id", with_paths=False) is ctx
...
def test_resolve_cli_project_keeps_exit_two_error_contract(monkeypatch, capsys):
    ...
    monkeypatch.setattr(lib_project, "resolve_project", fail)

    with pytest.raises(SystemExit) as exc_info:
        lib_project.resolve_cli_project("missing")
