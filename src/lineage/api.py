...
class LineageStore:
    ...
    def _recover_book_releases(db: sqlite3.Connection, root: Path) -> None:
        """Reconcile a running build only when CURRENT and its manifest verify."""
        ...
        self._db.execute(
            """INSERT INTO build_members(run_id, source_id, chapter_id, status)
               ...
               ON CONFLICT(run_id, source_id, chapter_id) DO UPDATE SET
                 status=excluded.status,
                 input_hash=CASE WHEN ? = '' THEN build_members.input_hash ELSE ? END,
                 output_hash=CASE WHEN ? = '' THEN build_members.output_hash ELSE ? END,
                 output_path=CASE WHEN ? = '' THEN build_members.output_path ELSE ? END""",
            (run_id, source_id, chapter_id, status,
             input_hash, input_hash, output_hash, output_hash, output_path, output_path),
        )
        self._db.commit()
    ...
    def record_book_release(self, run_id: str, source_ids: tuple[str, ...],
        ...
        """Atomically mark a verified book release and its source members."""
        if self.build_run(run_id) is None:
            raise KeyError(run_id)
