...
class SnapshotStore:
    ...
    Persists via safe_write (tmp + os.replace; AtomicContext-aware) and
    recovers from JSONDecodeError / OSError on load by starting with an
    empty dict.
    """
    ...
    def _save(self) -> None:
        """Persist via safe_write — atomic + AtomicContext-aware."""
        safe_write(
            self.path,
