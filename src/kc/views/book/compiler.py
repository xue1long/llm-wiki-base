...
from src.kc.views.book.id_policy import generate_knowledge_block_id
...
def _build_block_for_ku(
    ...
    *,
    ...
    The block id is generated by ``id_policy.generate_knowledge_block_id``
    with a slug derived from the KU's title (id-policy contract).
    """
    # id_policy requires a slug; derive from KU title (normalization handled
    # inside generate_knowledge_block_id). Falls through to "untitled" if
    # the title is empty.
    slug = str(getattr(ku, "title", "") or "")
    return KnowledgeBlock(
        id=generate_knowledge_block_id(slug),
