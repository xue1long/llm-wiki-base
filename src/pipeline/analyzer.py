...
Some source blocks may be intentionally omitted from the prompt after noise
filtering. Never cite an omitted block, an empty/zero-width block, or metadata
that is not visible above. If a claim has no visible verbatim quote, omit the
claim instead of fabricating an evidence entry.
"""


ANALYZER_JSON_PROMPT_V2 = """You are analyzing a source document to extract structured knowledge claims.
Return only a JSON object with source_id, type, title, and claims.
Each claim must contain statement, confidence, and evidence_block_ids.
For a non-empty source, claims MUST contain at least one substantive claim
(prefer 3-10 when the source supports them). An empty claims array is invalid.
Do not output evidence or quote fields: quote is generated by the system from
the canonical source block after this response.
Only select block IDs from the visible block registry in the source text.
If no visible block supports a claim, omit that claim.

source_id: {source_path}
folder_context: {folder_context}
existing_wiki_index: {existing_wiki_index}
schema: __PROJECT_SCHEMA__
purpose: __WIKI_PURPOSE__
taxonomy: __PROJECT_TAXONOMY__

Source text:
{chunk_context}{source_text}

JSON shape:
{{"source_id":"{source_path}","type":"concept","title":"title","claims":[{{"statement":"claim","confidence":0.0,"evidence_block_ids":["visible-block-id"]}}]}}
"""
...
_ANALYZER_JSON_RESPONSE_FORMAT_V2 = {
    "type": "object",
    "properties": {
        "source_id": {"type": "string"},
        "type": {"type": "string"},
        ...
        "claims": {
            ...
            "items": {
                ...
                "required": ["statement", "confidence", "evidence_block_ids"],
            },
        },
    },
    "required": ["source_id", "type", "title", "claims"],
...
async def _analyze_json(
    ...
    prompt_template = ANALYZER_JSON_PROMPT_V2 if evidence_contract == "v2" else ANALYZER_JSON_PROMPT
    response_format = (
        _ANALYZER_JSON_RESPONSE_FORMAT_V2
        if evidence_contract == "v2"
        else _ANALYZER_JSON_RESPONSE_FORMAT
    )
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        if evidence_contract == "v2" and not any(
            ...
            and item.get("statement", "").strip()
            and isinstance(item.get("evidence_block_ids"), list)
            and item.get("evidence_block_ids")
            for item in response.get("claims", [])
        ):
            last_error = "claims must contain at least one supported claim"
            _al.warning(
            ...
            if attempt == MAX_ATTEMPTS - 1:
                raise ValueError(
                    f"Analyzer returned no supported claims after {MAX_ATTEMPTS} attempts"
                )
            continue
    ...
    if evidence_contract == "v2":
        return CandidateV2(
            source_id=str(response.get("source_id", source_path)),
            type=str(response.get("type", "concept")),
            ...
            claims=tuple(
                ClaimV2(
                    ...
                    evidence_block_ids=tuple(
                        value for value in item.get("evidence_block_ids", [])
