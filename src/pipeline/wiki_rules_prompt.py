...
### 7.0 语义去重与引用边界（candidate render）

- 同一批次中相同 `id` 或同一 `type` 下相同标题只保留一个确定性页面；
  不按模糊相似度自动合并不同概念。
- 标题变体只有在 `SlugAliasRegistry` 中已有明确 alias → canonical 记录时
  才能归并；仅凭相似标题必须进入人工复核，不能写入第二个页面。
- 论据、收益、重要性或其他小节级内容默认并入所属概念；只有存在独立、
  可复用且证据充分的候选时才创建概念页。
- 普通引用只能指向本次响应已定义的页面、现有索引页或当前 source 页。
  不猜 slug、不造 pinyin 变体；taxonomy 命名空间是虚拟目标，不是待创建
  的普通 wiki 页面。
