...
(() => {
  ...
  App.showView = function showView(name) {
    ...
    const fn = { search: App.renderSearch, book: App.renderBook, browse: App.renderBrowse, ingest: App.renderIngest, collect: App.renderCollect, chat: App.renderChat, graph: App.renderGraph, heat: App.renderHeat, templates: App.renderTemplates, status: App.renderStatus }[name];
    if (fn) fn(content);
  };
  ...
  App.updateBreadcrumb = function updateBreadcrumb(path) {
    ...
    const labels = { search: "搜索", book: "Book", browse: "浏览", ingest: "摄取", collect: "采集", chat: "对话", graph: "图谱", heat: "热度", templates: "模板", status: "状态" };
