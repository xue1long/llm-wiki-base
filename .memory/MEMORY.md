...
- [2026-09-19 V7 Stage 3 ASR 豁免 Prompt 修复与新发现](feedback-v7-stage3-asr-prompt-fix-2026-09-19.md) — 5x 跑 4/5 个 ASR 转录稿在 Stage 3 判 incomplete；改 completeness.toml v1.1→v1.2 加 ASR 例外条款（commit `cdb274ed`），4 个新测试 + 26/26 通过；**但实测证明 prompt 改动不生效**——根因是 Stage 2 invariant I5 严格 byte accounting 让 ASR 稿全判 `status=degraded / boundary_confidence=0.5`，LLM 把这俩结构化信号当硬否决条件；候选修复 A/B/C 等用户决策
- [2026-09-19 V7 Stage 2 TAIL_RESIDUE + lineage UNIQUE 修复](feedback-v7-stage2-i5-lineage-fix-2026-09-19.md) — plan 2026-09-19-v7-stage2-i5-lineage-unblock Task 1/2 实施：Stage 2 新增 TAIL_RESIDUE 状态（保留 I5 严格性 + 新字段 + boundary_confidence 按 status 算，`101fde39`）；lineage helper `_safe_insert_artifact_sources` 去重 + INSERT OR IGNORE + FK 显式 log，4 处 executemany 全修，log 写失败兜底（`0c461a5f`）；5 跑实测 5/5 PASS；1188 pipeline 测试零回归；ADR-0015 决策；按 dev-relay 走 plan-audit 两轮 + 人工复核才进编码
- [2026-09-19 V7 code-review 阶段 3 双轴报告](feedback-v7-code-review-2026-09-19.md) — Standards + Spec 双轴 sub-agent 独立审计：Standards 2 hard violations（V1 lineage 跨层 import `from ..lib.errors` 违反 AGENTS.md 模块化永久约束；V2 cdb274ed fast-track 绕过 plan-audit 已记录反例） + 3 judgement calls；Spec 3 missing（M1 1025 边界 fixture 缺失 / M2 数量偏差 / M3 Stage 5 Open risks 不显式） + 3 scope creep + 3 looks wrong（UNIQUE 死代码 + UNIQUE 静默吞无测试 + run_v7_5x.py 整体 exit code 未留痕）；7 项 FIX + 4 项 ACCEPT
- [2026-09-20 Plan A 关系同步语义与反向边重构总结](feedback-relation-sync-fix-2026-09-19.md) — 阶段 4 架构体检衍生，将原庞大 plan 拆分为 Plan A（Bug 修复）后落地。完成 Task 1-4：修正 `sync_page` 语义为保留追加去重；重命名 `_compute_reverse_relations` 为 `_collect_inverse_relations` 并保留废弃别名；废弃 `resolve_wiki_target` 并沉淀 ADR-0017。全量测试通过无回归。
