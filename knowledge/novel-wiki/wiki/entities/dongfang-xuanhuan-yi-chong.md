---
id: dongfang-xuanhuan-yi-chong
title: 东方玄幻服饰文化体系
type: entity
sources:
- knowledge\novel-wiki\raw\sources\04_题材专题\东方玄幻各朝代服饰.md
created_at: 1785347256342
updated_at: 1785347256342
relations: []
grade: A
processing_depth: entity
is_immutable: false
heat: 50
last_used_at: 0
zombie_since: null
tags:
- entity/题材元素
---

<!-- wiki-template-version: 2.0.0 -->
<!-- wiki-template-type: entity -->

## 基本信息

{'category': 'Cultural Artifact', 'domain': 'Eastern Fantasy Worldbuilding'}

## 简介

东方玄幻中的服饰文化体系是构建世界观的核心组成部分。它不仅是角色的外在形象，更是传达种族、地位、魔法属性及历史背景的非语言符号。本实体整合了从上古至明清的历史数据，旨在为作者提供可考据的设定依据。

## 相关引用

- {'target': '[[world-building]]', 'type': 'is_part_of', 'weight': 0.9, 'context': '服饰是世界观构建的具体表现之一'}
- {'target': '[[dongfang-xuanhuan-shi-jie-guan-ce5facd6]]', 'type': 'related_to', 'weight': 0.8, 'context': '直接影响角色外观与文化设定'}
